const movieContainer = document.getElementById('movie-container'); // Make sure this matches the ID in your HTML
...
async function searchMovie() {
    const query = document.getElementById('search-input').value; // Ensure this matches the ID in your HTML
    const response = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=ef4ccb40d752e020b1a02f65cbb22a01&query=${encodeURIComponent(query)}`); // Corrected URL
    const data = await response.json();
